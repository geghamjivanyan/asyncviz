...
export function virtualize(inputs: VirtualizationInputs): VirtualizationOutput {
  const { frame, viewport, maxNodes = DEFAULT_MAX_NODES, maxEdges = DEFAULT_MAX_EDGES } = inputs;
  const clippedNodes = clipToViewport(frame.laidNodes, viewport, intersectsViewport);
  ...
  return {
    visibleNodes:
      nodeOverflow === 0 ? clippedNodes : clippedNodes.slice(clippedNodes.length - maxNodes),
    visibleEdges:
      edgeOverflow === 0 ? clippedEdges : clippedEdges.slice(clippedEdges.length - maxEdges),
    nodeOverflow,
    edgeOverflow,
  };
}
