/**
 * Frontend-side observability counters for the semaphore panel.
 *
 * Same pattern as the queue panel's metrics collector — singleton
 * process-wide; diagnostics page polls ``snapshot()``.
 */

export interface SemaphoreContentionPanelMetricsSnapshot {
  hydrations: number;
...
class SemaphoreContentionPanelMetrics {
  ...
  snapshot(): SemaphoreContentionPanelMetricsSnapshot {
    return {
...
const _instance = new SemaphoreContentionPanelMetrics();

export function getSemaphoreContentionPanelMetrics(): SemaphoreContentionPanelMetrics {
  return _instance;
}

export function resetSemaphoreContentionPanelMetrics(): void {
  _instance.reset();
}
