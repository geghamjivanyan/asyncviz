 ...
 */
export function sequenceToPixel(sequence: number, viewport: ReplayTimelineViewport): number {
...
export function pixelToSequence(pixel: number, viewport: ReplayTimelineViewport): number {
 ...
 */
export function sequenceToFraction(sequence: number, window: ReplaySessionWindow): number {
...
export function fractionToSequence(fraction: number, window: ReplaySessionWindow): number {
 ...
 */
export function timestampToSequence(monotonicNs: number, window: ReplaySessionWindow): number {
  const tsSpan = window.maxMonotonicNs - window.minMonotonicNs;
  if (tsSpan <= 0) return window.minSequence;
  const clamped = clamp(monotonicNs, window.minMonotonicNs, window.maxMonotonicNs);
...
export function sequenceToTimestamp(sequence: number, window: ReplaySessionWindow): number {
  const seqSpan = window.maxSequence - window.minSequence;
 ...
 */
export function sequenceInViewport(sequence: number, viewport: ReplayTimelineViewport): boolean {
  return sequence >= viewport.startSequence && sequence <= viewport.endSequence;
}
