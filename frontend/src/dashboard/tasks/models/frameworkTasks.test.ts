...
describe("isFrameworkTask", () => {
  ...
  it("flags Uvicorn / ASGI prefixes", () => {
    expect(
      isFrameworkTask({ coroutineName: "uvicorn.protocols.serve", taskName: null }),
    ).toBe(true);
    expect(
      isFrameworkTask({ coroutineName: null, taskName: "asgi.lifespan.startup" }),
    ).toBe(true);
  });
  ...
  it("flags AsyncViz internal bookkeeping tasks", () => {
    ...
    expect(
      isFrameworkTask({ coroutineName: "asyncviz.dashboard.runtime", taskName: null }),
    ).toBe(true);
  });
  ...
  it("does NOT flag user coroutines", () => {
    ...
    expect(
      isFrameworkTask({ coroutineName: "parent_with_children", taskName: "parent" }),
    ).toBe(false);
    expect(
      isFrameworkTask({ coroutineName: "semaphore_worker", taskName: "sem-worker-3" }),
    ).toBe(false);
  });
  ...
  it("does NOT flag user coroutines whose name happens to contain a substring elsewhere", () => {
    ...
    expect(
      isFrameworkTask({ coroutineName: "starletteAdmin", taskName: "starletteAdmin" }),
    ).toBe(false);
  });
});
