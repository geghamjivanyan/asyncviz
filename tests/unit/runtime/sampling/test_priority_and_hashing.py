...
def test_classify_critical_events() -> None:
    ...
    assert classify_event_priority("asyncio.loop.blocked") == SamplingPriority.CRITICAL


def test_classify_structural_events() -> None:
    assert classify_event_priority("asyncio.task.created") == SamplingPriority.STRUCTURAL
    assert classify_event_priority("asyncio.queue.created") == SamplingPriority.STRUCTURAL
    assert classify_event_priority("asyncio.gather.completed") == SamplingPriority.STRUCTURAL


def test_classify_delta_events() -> None:
    assert classify_event_priority("asyncio.queue.metrics.updated") == SamplingPriority.DELTA
    assert classify_event_priority("runtime.metric") == SamplingPriority.DELTA


def test_classify_falls_back_to_state() -> None:
    assert classify_event_priority("asyncio.task.waiting") == SamplingPriority.STATE
...
def test_deterministic_bucket_changes_with_sequence() -> None:
    ...
    buckets = {deterministic_bucket("asyncio.task.created", i) for i in range(16)}
